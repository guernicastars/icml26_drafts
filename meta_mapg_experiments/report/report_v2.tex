...
\subsection{Non-tabular Check: MLP Policies in IPD}
\label{sec:mlp_check}

To verify that the computational advantage of Meta-MAPG is not a tabular artefact, we repeat the ablation on the iterated prisoner's dilemma (IPD) using $2$-layer MLP policies ($16$ hidden units, tanh). This scales the parameter space from $5$ to $\approx\!385$ dimensions per agent. We use the same hyperparameters ($\lambda_c=1.5, \eta=0.35$) and batch size ($384$) as in the tabular experiments. 

While the $260$-step budget is insufficient for full convergence to the cooperative Nash (0\% formal success), we observe a sharp qualitative separation in outcome metrics. Standard policy gradient and Meta-PG reliably collapse to the defective Nash (1.0 return/step). In contrast, Meta-MAPG and LOLA successfully resist this collapse in $13\%$ and $3\%$ of seeds respectively. Quantitatively, Meta-MAPG achieves a $4\times$ higher mean cooperation rate ($6.7\%$ vs.\ $1.7\%$) and a significantly higher mean return ($11.6$ vs.\ $10.2$) compared to the peer-only LOLA baseline. This confirms that the spectral reinforcement effect of opponent-aware shaping persists under modest neural parameterization, providing a "proof-of-transport" for the theoretical framework to deep MARL.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{figs/mlp_ipd.pdf}
  \caption{Non-tabular IPD check. Cooperative success rate (coop $\ge 0.82$) on IPD with 2-layer MLP policies (30 seeds). Peer-aware methods (Meta-MAPG, LOLA) show a clear qualitative advantage over non-peer methods (PG, Meta-PG) at the same coefficients as the tabular experiments. While the 260-step budget yields zero formal convergence, Meta-MAPG provides 4$\times$ higher mean cooperation than LOLA.}
  \label{fig:mlp_ipd}
\end{figure}
...
\begin{enumerate}[itemsep=3pt]
  ...
  \item \textbf{Spectral reinforcement has a sharp boundary.}
    ...
    over Own-only. Crucially, the non-tabular IPD check demonstrates
    that even with MLP policies, peer-aware shaping provides a 
    qualitative advantage in resisting defective collapse, achieving
    a $4\times$ improvement in mean cooperation over LOLA within 
    the same step budget. This confirms the theory "transports" 
