...
def main():
    ...
    run_dir = setup_run_dir(args.output_dir, experiment="polymarket", run_name=args.run_name)
    ...
    np.save(data_dir / "categories.npy", feats.categories)
    np.save(data_dir / "condition_ids.npy", feats.condition_ids)
    with open(data_dir / "feature_names.txt", "w") as f:
        f.write("\n".join(feats.feature_names))

    rows = []
    for latent in tqdm(args.latent_dims, desc="polymarket latents", unit="dim"):
    ...
    summary.to_csv(run_dir / "polymarket_embedding.csv", index=False)

    dump_metadata(run_dir, {
        "n_markets": int(X_std.shape[0]),
