...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("results/art"))
    ...
    pd.DataFrame(results).to_csv(args.output_dir / "art_embedding.csv", index=False)
    np.save(args.output_dir / "features_X.npy", feats.X)
    np.save(args.output_dir / "features_X_std.npy", X_std)
    np.save(args.output_dir / "sources.npy", feats.sources)
