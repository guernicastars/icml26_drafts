...
\begin{axis}[
  ...
  \addplot+[no marks, thick, color=green!60!black, dashed]
    table[x=step, y=d_eff] {data/grokking_trajectory.dat};
  \addlegendentry{$\deff$ (penultimate)}
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
  ...
  \addplot+[mark=square, mark size=2pt, thick, color=orange!80!black, dashed]
    table[x=n_noise_dims, y=d_eff_mean] {data/noise_dim.dat};
  \addlegendentry{$\deff$ (latent)}
\end{axis}
\end{tikzpicture}
