...
\bottomrule
\end{tabular}
\caption{Noise-reservoir test across Pythia models.
$\sinT < 0.04$ for all models at $m=500$ equal-variance noise columns;
Davis--Kahan bound is 0.10--0.12.
The signal-to-noise gap $\lambda_{10}/\sigma_z^2$ ranges from 7 to 70,
confirming that the spectral gap condition of
Theorem~\ref{thm:noise-reservoir} is satisfied.}
\label{tab:nr}
\end{table}
...
reconstruct Pythia-160M final hidden states $H$ ($n=20{,}000$ samples,
$\deff = 83.4$).
UET predicts a soft knee in reconstruction MSE near $k = \deff$:
below $\deff$, the bottleneck discards signal subspace directions; above,
the loss should plateau.
...
\addplot[dashed, thick, color=orange!80!black] coordinates {(128,0)(128,1)};
\addlegendentry{min-MSE at $k=128$}
\addplot[dashed, thick, color=red!60!black] coordinates {(83,0)(83,1)};
\addlegendentry{$\deff = 83.4$}
\end{axis}
\end{tikzpicture}
\caption{Relative MSE of $k$-bottleneck reconstruction of Pythia-160M hidden
states ($\deff = 83.4$).
MSE decreases smoothly with $k$ and plateaus near $k \approx 64$--$128$ (below
the $\deff$ threshold), with the minimum at $k=128$ ($\text{rel-MSE} = 0.016$).
The bottleneck latent $\deff$ peaks at $\approx 13$ (not 83), indicating
that the MLP learns a compressed representation whose intrinsic dimension is
much lower than the teacher's $\deff$.
The gradual curve (not a sharp knee) is consistent with Pythia-160M's
power-law spectrum ($k_{90\%} = 479$) rather than a step-function subspace.}
\label{fig:distill}
\end{figure}
